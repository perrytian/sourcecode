...
public class HttpImpl implements Http{
	...
	public HttpImpl(String url){
		GET(url);
	}
	
	/**
	 * Get 方法根据URL，会请求文件，数据库查询结果，程序运行结果等多种内容
	 */
	public void GET(String url){
		try {
			...
			sendMessage(cmd);
			System.out.println("HTTP协议： 执行GET方法\n"+cmd);
			receiveMessage();
    ...
    protected void receiveMessage() throws IOException{   
        ...
        serverVersion= message.substring(0,mark);   //HTTP/1.0
        ...
        responseCode = Integer.parseInt(message.substring(mark + 1,mark += 4));//200   
  
        responseMessage = message.substring(mark,message.length()).trim();   //OK
        ...
        while(word != '\r' && word != '\n' && word > -1){   
            ...
            message = new String(data,0,count);   //Date:Web,28 Apr 2010 07:04:23 GMT
            ...
            String value = message.substring(mark,message.length());   //时间
    ...
    protected String getBaseHeads(){   
        ...
        String inf = "User-Agent: cusi cnms httpAgent\r\nAccept: www/source; text/html; image/gif; */*\r\n";   
    ...
    public static void main(String[] args){
    	...
    	ByteArrayInputStream input = (ByteArrayInputStream) http.getInputStream();
    	int end = 0 ;
    	int count = 0;
	...
	public int getResponseTime() {
		System.out.println("HTTP getResponseTime未实现");
		return 0;
	}
